...
- Set Storage and Observability to start collapsed; the other groups start expanded.
...
- Updated the status bar to show the connected cluster on the left and kubectl, Docker, and kind detection on the right, each with independent green, red, and checking indicators.
...
- Generated numbered snapshots for the Dashboard, all 17 resource views, and the unavailable-terminal state in [docs/snapshots](docs/snapshots).
...
- The final Playwright end-to-end run passed all 29 tests, including video recording and GIF generation.
